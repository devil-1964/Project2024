...
const createSamplePlacementData = async () => {
    try {
        ...
        console.log(`📊 Found ${existingRecords.length} existing placement record(s)`);

        // Step 2: Delete all existing placement records
        const deletedRecords = await PlacementYear.deleteMany({});
        console.log(`🗑️  Deleted ${deletedRecords.deletedCount} placement record(s)`);
        ...
        for (const yearData of samplePlacementData) {
            ...
            console.log(`✅ Created placement record for year ${yearData.year} with ${yearData.branches.length} branches`);
            
            // Log some statistics for each year
            const totalStudents = yearData.branches.reduce((sum, branch) => sum + branch.eligibleInterestedStudents, 0);
            const totalPlacements = yearData.branches.reduce((sum, branch) => sum + branch.totalPlacementIncludingHigherEducation, 0);
            const overallPercentage = ((totalPlacements / totalStudents) * 100).toFixed(1);
            console.log(`   📈 Overall placement: ${totalPlacements}/${totalStudents} (${overallPercentage}%)`);
        }

        // Summary
        console.log('\n🎉 Sample placement data creation completed successfully!');
        console.log('📋 Summary:');
        console.log(`   📅 Years covered: ${samplePlacementData.map(y => y.year).join(', ')}`);
        console.log(`   📚 Branches: Computer Science, Electrical Engineering, Mechanical Engineering, Civil Engineering, Electronics`);
        ...
        samplePlacementData.forEach(yearData => {
            const yearStudents = yearData.branches.reduce((sum, branch) => sum + branch.eligibleInterestedStudents, 0);
            const yearPlacements = yearData.branches.reduce((sum, branch) => sum + branch.totalPlacementIncludingHigherEducation, 0);
            grandTotalStudents += yearStudents;
            grandTotalPlacements += yearPlacements;
        });
        
        const grandOverallPercentage = ((grandTotalPlacements / grandTotalStudents) * 100).toFixed(1);
        console.log(`   🎯 Grand total placement: ${grandTotalPlacements}/${grandTotalStudents} (${grandOverallPercentage}%)`);

    } catch (error) {
        console.error('❌ Error during sample placement data creation:', error.message);
        throw error;
    }
};
...
const addPlacementYear = async (year, branchesData) => {
    try {
        ...
        const totalStudents = branchesData.reduce((sum, branch) => sum + branch.eligibleInterestedStudents, 0);
        const totalPlacements = branchesData.reduce((sum, branch) => sum + branch.totalPlacementIncludingHigherEducation, 0);
        const overallPercentage = ((totalPlacements / totalStudents) * 100).toFixed(1);
        console.log(`   📈 Overall placement: ${totalPlacements}/${totalStudents} (${overallPercentage}%)`);
        ...
        console.error(`❌ Error adding placement data for year ${year}:`, error.message);
        throw error;
    }
};
...
const main = async () => {
    try {
        ...
        if (args.length === 0) {
            ...
            console.log('📖 Usage:');
            console.log('  node samplePlacementData.js              Create sample placement data (replaces existing)');
            console.log('  node samplePlacementData.js --help       Show this help message');
            console.log('');
            console.log('💡 This script creates sample placement data for years 2021, 2022, and 2023');
