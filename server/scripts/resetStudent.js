...
const resetStudents = async () => {
    try {
        ...
        console.log(`🗑️  Deleted ${deletedStudentDetails.deletedCount} student detail record(s)`);
        ...
        for (let i = 0; i < sampleStudents.length; i++) {
            ...
            console.log(`✅ Created student user: ${studentData.name} (ID: ${newStudentUser._id})`);
        ...
        sampleStudents.forEach((student, index) => {
            console.log(`   ${index + 1}. ${student.name} (${student.email}) - ${student.branch} - Batch ${student.batchYear}`);
        });
...
const createStudent = async (username, email, password, name, branch, batchYear, phone, semCgpa, activeBacklogs = 0) => {
    try {
...
const main = async () => {
    ...
    try {
        if (command === 'reset') {
            ...
            const [, username, email, password, name, branch, batchYear, phone, semCgpa, activeBacklogs] = args;
            
            if (!username || !email || !password || !name || !branch || !batchYear || !phone) {
                console.log('❌ Usage: node resetStudent.js create <username> <email> <password> <name> <branch> <batchYear> <phone> [semCgpa] [activeBacklogs]');
                console.log('📝 Example: node resetStudent.js create john2025 john@student.com pass123 "John Doe" "Computer Science" 2025 9876543210 "8.5,8.7,8.9" 0');
                process.exit(1);
            }

            await createStudent(username, email, password, name, branch, batchYear, phone, semCgpa, activeBacklogs);
        } else {
            console.log('📖 Usage:');
            console.log('  Reset all students:   node resetStudent.js reset');
            console.log('  Create new student:   node resetStudent.js create <username> <email> <password> <name> <branch> <batchYear> <phone> [semCgpa] [activeBacklogs]');
            console.log('\n📝 Examples:');
            console.log('  node resetStudent.js reset');
            console.log('  node resetStudent.js create john2025 john@student.com pass123 "John Doe" "Computer Science" 2025 9876543210 "8.5,8.7,8.9" 0');
        }
    } catch (error) {
